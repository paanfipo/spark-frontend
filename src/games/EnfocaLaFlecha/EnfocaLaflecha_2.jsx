...
import { consultarEntrenadorIA } from '../../services/iaService';
...
export default function EnfocaLaFlecha({ onGameOver }) {
  ...
  // --- FUNCIÓN PARA LA IA ---
  const llamarEntrenadorIA = async (metrics) => {
    const datosParaIA = {
      nombre_juego: "Enfoca la Flecha",
      aciertos: metrics.tasa_aciertos,
      comisiones: metrics.errores_comision,
      omisiones: metrics.errores_omision,
      tr_medio: metrics.tiempo_respuesta_promedio_ms
    };

    // 2. Llamamos al servicio que creamos
    const guiaIA = await consultarEntrenadorIA(datosParaIA);

    // 3. Mostramos el mensaje (puedes cambiar el alert por un modal más adelante)
    if (guiaIA) {
      setTimeout(() => {
      alert("CONSEJO DEL ENTRENADOR IA:\n" + guiaIA.mensaje);
    }, 500);
    }
  };
      ...
      const timer = setInterval(() => {
        setTimeLeft(prev => {
          ...
          if (newTime <= 0) {
            ...
            llamarEntrenadorIA(finalMetrics);
            return 0;
          }
          return newTime;
        });
