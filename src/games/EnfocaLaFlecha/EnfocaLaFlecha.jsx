...
export default function EnfocaLaFlecha({ onGameOver }) {
  ...
  const calculateFinalMetrics = useCallback(() => {
    // Si no hubo actividad, devolvemos valores por defecto para evitar errores
    ...
    // Retornamos el objeto con nombres de campos EXACTOS
    return {
      ...
      score: score, // Importante: GamePlayer suele buscar este campo
  ...
  const handleInput = useCallback((dir) => {
    ...
    if (isCorrect) {
      ...
      setLevelStats(prev => [...prev, { correct: true, rt, level }]);
      
      if ((levelStats.length + 1) % 7 === 0) {
      ...
      setLevelStats(prev => [...prev, { correct: false, rt, level }]);
      generateTrial();
